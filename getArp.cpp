...
int main()
{
	...
	local.nl_groups = 	RTMGRP_LINK | RTMGRP_IPV4_IFADDR | RTMGRP_IPV4_ROUTE | RTMGRP_NEIGH | RTMGRP_IPV6_IFADDR | RTMGRP_IPV6_ROUTE;	// указываем необходимые группы
	local.nl_pid = 0;
	...
	while (1) {
		...
		for (h = (struct nlmsghdr*)buf; status >= (ssize_t)sizeof(*h); ) {	// для всех заголовков сообщений
			...
			if ((h->nlmsg_type == RTM_NEWROUTE) || (h->nlmsg_type == RTM_DELROUTE) || (h->nlmsg_type == RTM_GETNEIGH)) {	// если это изменения роутов - печатаем сообщение
                ...
                cout << "Type---->" << h->nlmsg_type << endl;
                cout << RTM_GETADDR << endl;


                struct ndmsg* pNdmsg;
                struct nda_cacheinfo* pCache;
                pCache
                pNdmsg = (struct ndmsg*)NLMSG_DATA(h);
                parseRtattr(tba, IFA_MAX, IFA_RTA(pNdmsg), h->nlmsg_len);
                if(tba[NDA_CACHEINFO]){cout << "CACHEINFO" << endl;}
